...
type unarchiveTest struct {
	dstDir string
	name   string
	mode   os.FileMode
	data   []byte
	files  []unarchiveTestFile
}

type unarchiveTestFile struct {
	name     string
	modified time.Time
	content  []byte
}

var unarchiveTests = []unarchiveTest{
	{
		dstDir: "xyz",
		name:   "abc.zip",
		mode:   0644,
...
func TestUnarchive(t *testing.T) {
	...
	for _, tc := range unarchiveTests {
		testUnarchiveFile(t, fs, tc)
	}
}

func testUnarchiveFile(t *testing.T, fs afero.Fs, tc unarchiveTest) {
...
type unarchiveDirTest struct {
	data  []byte
	files []string
}

var unarchiveDirTests = []unarchiveDirTest{
	{
...
func TestUnarchiveDir(t *testing.T) {
	fs := afero.NewMemMapFs()
	for _, tc := range unarchiveDirTests {
		testUnarchiveDirFile(t, fs, tc)
	}
}

func testUnarchiveDirFile(t *testing.T, fs afero.Fs, tc unarchiveDirTest) {
	srcFile := "input.zip"
